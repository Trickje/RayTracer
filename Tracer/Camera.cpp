...
TRay TCamera::shootRay(const int& a_X, const int& a_Y, const vec2& a_ScreenDimensions)
{
	...
	return TRay(m_Position, (m_View * vec3(xPos, yPos, 1).normalize()));
	
}

TCamera::TCamera(mat4 lookAt, vec3 worldSpacePos)
{
...
vec3 const& TCamera::getPos() const
{
	return m_Position;
}
